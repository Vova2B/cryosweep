...
def _write_instrument_sigma(tmp_path, name):
    rows = []
    for b in np.arange(-20000.0, 20000.1, 200.0):
        rxy = 1e-3 + 5e-4 * (b / 1e4)
        rows.append(f"10.0000,{b:.1f},{rxy:.10e},{rxy / 1000:.10e},"
                    f"{_INSTSIG_SD:.10e},{1e-3:.10e},{1e-6:.10e}")
    rows += [f"{t:.4f},20000.0,1.5e-3,1.5e-6,{_INSTSIG_SD:.10e},1e-3,1e-6"
...
def test_real_file_ladder_is_quiet(hall_real_path):
    ...
    MEASURED (2026-09): on the real Hall file (hall_channel=1, thickness=0.07 mm,
    longitudinal_channel=2 -- the same config test_real_file_field_sweep_declines_nothing
    uses), 3*sigma dominates the spread at all 9 temperatures by 10x-100x, including
    200 and 300 K, so window_sensitive fires NOWHERE on this sample. That is the
    informative result for a linear sample, not a sign the flag is dead code -- it says
    the fit window does not move R_H here, which is itself worth being able to state.
    """
